...
function M._process_queued_mentions()
  ...
  vim.defer_fn(function()
    ...
    -- Terminal auto-open disabled - process queued mentions without showing terminal
...
---@brief Show terminal if Claude is connected and it's not already visible (DISABLED)
---@return boolean success Whether terminal was shown or was already visible
function M._ensure_terminal_visible_if_connected()
  -- Terminal auto-open functionality disabled
  return false
end
...
function M.send_at_mention(file_path, start_line, end_line, context)
  ...
  if M.is_claude_connected() then
    -- Claude is connected, send immediately (terminal auto-open disabled)
    local success, error_msg = M._broadcast_at_mention(file_path, start_line, end_line)
    return success, error_msg
  else
    -- Claude not connected, queue the mention (terminal auto-open disabled)
    local mention_data = {
      file_path = file_path,
    ...
    logger.debug(context, "Queued @ mention (terminal auto-open disabled): " .. file_path)
...
function M._create_commands()
  ...
  if terminal_ok then
    vim.api.nvim_create_user_command("ClaudeCode", function(opts)
      -- Terminal functionality disabled - do nothing
      return
    end, {
      nargs = "*",
